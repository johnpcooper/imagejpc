...
// Begin jpc macros
// For some plugins I've written, if I try to run them in a macro using
// run("name of plugin py file")
// It just opens the plugin in the Fiji IDE. But if I run it like I'm
// searching the command in the search bar it actually runs the plugin
// See "save cell roi set" for an example

macro "Sort ROIs [S]" {
	roiManager("Sort");
}

macro "Threshold brightfield [Q]" {
...
macro "name round roi [6]" {
	run("name round roi")
}

macro "name long roi [7]" {
	run("name long roi")
}
